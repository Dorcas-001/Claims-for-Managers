...
def display_product():
    ...
    with col4:
        ...
        selected_years = st.multiselect(
            ...
            options=years,
            key="employer_selector_multiselector"
        )
        if selected_years:
    ...
    with col1:
        date1 = pd.to_datetime(display_date_input(col1, "Start Date", startDate, startDate, endDate, key="date_start_multiselector"))

    with col2:
        date2 = pd.to_datetime(display_date_input(col2, "End Date", endDate, startDate, endDate, key="date_end_multiselector"))
    ...
    with col2:
        ...
        for idx, product in enumerate(product_claim_count['Product']):
            fig_claim_status_by_product.add_trace(go.Bar(
                ...
                name=product,  # Product name as legend
                text=product_claim_count.loc[product_claim_count['Product'] == product, 'Claim Amount'].apply(lambda x: f"{x / 1_000:.1f}M"),  # Format values in millions
                textposition='inside',
                textfont=dict(color='white'),
                hoverinfo='x+y+name',
                marker_color=custom_colors[idx % len(custom_colors)]  # Cycle through custom colors
